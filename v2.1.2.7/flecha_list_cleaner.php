...
function flecha_clean_lists(){
	...
	switch($selection){
		case "openedorclicked":
			if(isset($cllistid)){
				//1 list
				$sql = "SELECT subid FROM flecha_activity_log WHERE ownerid = '$ownerid' AND listid = '$cllistid' AND (last_opened < '$date' OR last_clicked < '$date')";
			}else{
				//multiple list
				$sql = "SELECT subid FROM flecha_activity_log WHERE ownerid = '$ownerid' AND listid IN $cllistin AND (last_opened < '$date' OR last_clicked < '$date')";					
			}
		...
		case "opened":
			if(isset($cllistid)){
				//1 list
				$sql = "SELECT subid FROM flecha_activity_log WHERE ownerid = '$ownerid' AND listid = '$cllistid' AND last_opened < '$date'";
			}else{
				//multiple list
				$sql = "SELECT subid FROM flecha_activity_log WHERE ownerid = '$ownerid' AND listid IN $cllistin AND last_opened < '$date'";					
			}
			$response['debug1'] = __LINE__." Sql: $sql";				
			break;					
		case "clicked":
