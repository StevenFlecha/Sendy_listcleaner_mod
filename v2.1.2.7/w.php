...
//MOD EDIT BEGIN========================
	if(count($i_array)==4 && $i_array[3]=='a')
		$q = 'SELECT campaign_id,ares_emails_id FROM links WHERE link = "'.APP_PATH.'/w/'.$i_array[2].'/a"';
	else
		$q = 'SELECT campaign_id,ares_emails_id FROM links WHERE link = "'.APP_PATH.'/w/'.$i_array[2].'"';
	$r = mysqli_query($mysqli, $q);
	$row = mysqli_fetch_array($r);
	if(is_numeric($row['campaign_id'])){
		//bit faster gettin owner id
		$q = 'SELECT to_send_lists,userID FROM campaigns WHERE id = '.$row['campaign_id'];
		$r = mysqli_query($mysqli, $q);
		$row = mysqli_fetch_array($r);
		$ownerid = $row['userID'];
		...
		$q = 'SELECT l.userID, l.id FROM lists AS l
			...
			WHERE ar.id = '.$row['ares_emails_id'];
		$row = mysqli_fetch_array($r);
		$ownerid = $row['userID'];
		$listids = $row['id'];			
	}else{
